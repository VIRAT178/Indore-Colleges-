...
async function seedDatabase(db: Db) {
  try {
    ...
    console.log('[MongoDB] Note during database seed check:', err?.message || err);
  }
}
...
async function getMongoDb(): Promise<Db> {
  ...
  if (!uri || /<password>|<username>|username:password|your_mongo|your_cluster|example\.com|placeholder|dummy/i.test(uri)) {
    throw new Error('MONGODB_URI must be configured with a real MongoDB connection string.');
  }
...
export async function startServer() {
  ...
  app.post('/api/counseling', async (req, res) => {
    ...
    try {
      ...
      const requestEmail = newRequest.email;
      if (requestEmail) {
