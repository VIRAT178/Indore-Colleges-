      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 shadow-sm">
        ...
        {loggedInCollege !== null && activeTab === 'dashboard' && (
          <div className="space-y-8">
            ...
            {/* PENDING MODERATION UPDATE BANNER */}
            {loggedInCollege.pendingUpdate && (
              <div className="p-4 sm:p-5 bg-amber-50/70 border border-amber-200 rounded-2xl">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="text-xs font-black text-amber-900 uppercase tracking-wider">
                        Profile Update Request Pending Admin Review
                      </h4>
                      <span className="text-[10px] font-bold bg-amber-200/80 text-amber-950 px-2 py-0.5 rounded-full">
                        Submitted on {new Date(loggedInCollege.pendingUpdate.createdAt).toLocaleDateString()}
                      </span>
                    </div>
                    <p className="text-xs text-amber-800 leading-relaxed">
                      Your requested modifications (fee changes, facilities, address, or descriptions) have been forwarded to the <strong>Indore Colleges Admin Team</strong>. 
                      Once verified and approved by the admin, your revised college profile will be updated and published live to students immediately.
                    </p>
                    {loggedInCollege.pendingUpdate.requestedChanges?.feePerAnnum !== undefined && (
                      <p className="text-[11px] font-bold text-amber-950 pt-1">
                        Proposed New Annual Fee: ₹{Number(loggedInCollege.pendingUpdate.requestedChanges.feePerAnnum).toLocaleString('en-IN')} (Under Review)
                      </p>
                    )}
                  </div>
                </div>
              ...
              <form onSubmit={handleUpdate} className="space-y-6">
                ...
                <div className="space-y-2">
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-red-600 hover:bg-red-500 text-white rounded-xl py-3 text-xs font-bold transition cursor-pointer flex items-center justify-center gap-2 shadow-xs"
                  >
                    {loading ? (
                      <>
                        <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent animate-spin rounded-full" />
                        <span>Submitting Update Request to Admin...</span>
                      </>
                    ) : (
                      <>
                        <Check className="h-4 w-4" />
                        <span>Send Update Request to Admin for Approval & Publishing</span>
                      </>
                    )}
                  </button>
                  <p className="text-[11px] text-slate-400 text-center">
                    ℹ️ In accordance with platform guidelines, college information updates are reviewed by the Indore Colleges Admin before being published live.
                  </p>
                </div>
              </form>
            )}
